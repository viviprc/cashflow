describe('Testeando el crud de inventario', () => {
    ...
    it('Probando crud de inventario',
        () => {
            ...
            cy.contains('td','Llave Stillson 8" acero', {matchCase: false}).siblings('td').last().children('button').first().click()
            cy.get('#stockProducto').type('{backspace}0')
            cy.get('#guardar').click()

            cy.contains('td','Llave Stillson 8" acero', {matchCase: false}).siblings('td').last().children('button').last().click()
            cy.get('#okBorrar').click()
            
            cy.contains('td','Llave Stillson 8" acero', {matchCase: false}).should('not.exist')
